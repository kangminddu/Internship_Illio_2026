...
def save_emails(cur, creator_id, channel_id, emails):
    """공개 설명란에서 추출한 이메일 저장. 중복(creator+email)은 무시."""
    if not emails:
        return 0
    n = 0
    for em in emails:
        # 과도하게 긴 값/명백한 오탐 방어 (255 초과, 점 없는 것 등)
        if len(em) > 255 or "." not in em.split("@")[-1]:
            continue
        cur.execute("""
            INSERT INTO creator_emails
              (creator_id, channel_id, email, source_platform, collected_at)
            VALUES (%s, %s, %s, 'youtube', %s)
            ON DUPLICATE KEY UPDATE
              channel_id = VALUES(channel_id),
              collected_at = VALUES(collected_at)
        """, (creator_id, channel_id, em, datetime.now()))
        n += 1
    return n
...
def process_one(channel):
    ...
    try:
        with conn.cursor() as cur:
            if r.ok:
                ...
                # 3.5) 공개 설명란 이메일 저장 (reCAPTCHA 없이 노출된 것만)
                n_email = save_emails(cur, creator_id, channel_id, r.emails)
                if n_email:
                    with lock:
                        counter["emails"] += n_email
    ...
    if d % 50 == 0:
        print(f"  [{d}] ok={counter['ok']} fail={counter['fail']} "
              f"rl={counter['rate_limited']} emails={counter['emails']}")
...
def main():
    ...
    print(f"\n=== 완료: ok={counter['ok']} fail={counter['fail']} "
          f"rate_limited={counter['rate_limited']} emails={counter['emails']} | {elapsed:.0f}초 ===")
    if stop_flag.is_set():
